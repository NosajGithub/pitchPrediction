...
Make_pitcher_batter_priors <- function(pdata){
        ...
        pdata[, -which(names(pdata) %in% c(paste0(unique_pitches),paste0(unique_pitches,"_b"))]
        
        unique_pitches_v <- paste0(unique(verlander2$pitch_type)) #paste0 for NAs
        verlander2 <- verlander2[, -(which(names(verlander2) 
                                           %in% c(paste0(unique_pitches_v),paste0(unique_pitches_v,"_b"),"total","total_b")))]
}
